...
#define DEFINE_LOGLEVEL(name) mrb_define_const(mrb, req, #name, mrb_fixnum_value(name))
...
static mrb_value minim_get_request_body(mrb_state *mrb, mrb_value self) {
  ...
  int len;
  request_rec *r = minim_get_request();
  if (r->method_number == M_POST || r->method_number == M_PUT || r->method_number == M_PATCH) {
    if (ap_setup_client_block(r, REQUEST_CHUNKED_ERROR) != 0) {
      mrb_sys_fail(mrb, "ap_setup_client_block");
    }
    if (!ap_should_client_block(r)) return mrb_nil_value();
